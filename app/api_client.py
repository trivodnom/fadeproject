...
def get_matches_for_league(league_id):
    ...
    params = {'league': league_id, 'season': datetime.utcnow().year, 'status': 'NS'}
...
# ===== НОВАЯ ФУНКЦИЯ ДЛЯ ПОЛУЧЕНИЯ ЛОГОТИПА =====
def get_team_logo_url(team_id):
    """Получает URL логотипа команды по ее ID."""
    api_host = current_app.config['API_HOST']
    api_key = current_app.config['API_KEY']
    params = {'id': team_id}
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json().get('response', [])
        if data and data[0].get('team', {}).get('logo'):
            return data[0]['team']['logo']
        return None
    except requests.exceptions.RequestException as e:
        print(f"API request for team logo failed for team {team_id}: {e}")
        return None